...
allAccordions.forEach(accordionContainer => {
    ...
    accordionContainer.addEventListener('click', (e) => {
        ...
        allItemsInThisAccordion.forEach(item => {
            ...
            if (itemContent) {
                ...
                itemContent.classList.remove('mb-4');
            }
    ...
    if (firstItem) {
        const firstItemContent = firstItem.querySelector('.accordion-content');
        const firstItemIcon = firstItem.querySelector('.accordion-trigger img');

        if (firstItemIcon) {
            firstItemContent.style.maxHeight = firstItemContent.scrollHeight + 'px';
            firstItemContent.classList.add('mt-2');
        }

        if (firstItemIcon) firstItemIcon.classList.add('rotate-180');
    }
});